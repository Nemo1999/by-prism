---
title: "Lemon - 完整歌詞"
description: "米津玄師《Lemon》完整歌詞，每句歌詞連結到詳細解析，包含發音播放功能"
pubDate: 2026-03-21
author: 小波
tags: ["歌詞", "Lemon", "米津玄師", "完整歌詞", "發音功能"]
draft: false
---

# Lemon - 完整歌詞

> 米津玄師《Lemon》完整日文歌詞與中文翻譯

## 🔗 相關連結
- **學習專案**：[[posts/japanese-learning/lemon-song/index|《Lemon》學習專案]]
- **單字文法複習**：[[posts/japanese-learning/lemon-song/vocabulary-grammar-review|《Lemon》單字與文法總複習]]
- **學習進度**：[[posts/japanese-learning/lemon-study-progress|Lemon學習進度]]

## 🎵 日文歌詞（點擊播放按鈕可聽發音）

### Verse 1
1. [[posts/japanese-learning/lyrics/lemon-yume-naraba|夢ならばどれほどよかったでしょう]] <button class="pronunciation-play-btn" data-text="夢ならばどれほどよかったでしょう">🔊 播放</button>
   [[posts/japanese-learning/lyrics/lemon-imadani-anata-no-koto-wo-yume-ni-miru|未だにあなたのことを夢にみる]] <button class="pronunciation-play-btn" data-text="未だにあなたのことを夢にみる">🔊 播放</button>
   忘れた物を取りに帰るように <button class="pronunciation-play-btn" data-text="忘れた物を取りに帰るように">🔊 播放</button>
   [[posts/japanese-learning/lyrics/lemon-furubita-omoide-no-hokori-wo-harau|古びた思い出の埃を払う]] <button class="pronunciation-play-btn" data-text="古びた思い出の埃を払う">🔊 播放</button>

### Chorus
[[posts/japanese-learning/lyrics/lemon-modoranai-shiawase-ga-aru-koto-wo|戻らない幸せがあることを]] <button class="pronunciation-play-btn" data-text="戻らない幸せがあることを">🔊 播放</button>
[[posts/japanese-learning/lyrics/lemon-saigo-ni-anata-ga-oshiete-kureta|最後にあなたが教えてくれた]] <button class="pronunciation-play-btn" data-text="最後にあなたが教えてくれた">🔊 播放</button>
言えずに隠してた昏い過去も <button class="pronunciation-play-btn" data-text="言えずに隠してた昏い過去も">🔊 播放</button>
あなたがいる光になる <button class="pronunciation-play-btn" data-text="あなたがいる光になる">🔊 播放</button>

### Verse 2
雨が降り止むまでは帰れない <button class="pronunciation-play-btn" data-text="雨が降り止むまでは帰れない">🔊 播放</button>
今でもあなたはわたしの光 <button class="pronunciation-play-btn" data-text="今でもあなたはわたしの光">🔊 播放</button>
暗闇であなたの背をなぞった <button class="pronunciation-play-btn" data-text="暗闇であなたの背をなぞった">🔊 播放</button>
その輪郭を鮮明に覚えている <button class="pronunciation-play-btn" data-text="その輪郭を鮮明に覚えている">🔊 播放</button>

### Bridge
受け止めきれないものと出会うたび <button class="pronunciation-play-btn" data-text="受け止めきれないものと出会うたび">🔊 播放</button>
溢れてやまない藍色 <button class="pronunciation-play-btn" data-text="溢れてやまない藍色">🔊 播放</button>
あの日見渡した街並み <button class="pronunciation-play-btn" data-text="あの日見渡した街並み">🔊 播放</button>
響く靴音のない世界 <button class="pronunciation-play-btn" data-text="響く靴音のない世界">🔊 播放</button>

### Outro
レモンの匂い <button class="pronunciation-play-btn" data-text="レモンの匂い">🔊 播放</button>
雨が降る <button class="pronunciation-play-btn" data-text="雨が降る">🔊 播放</button>
夜の匂い <button class="pronunciation-play-btn" data-text="夜の匂い">🔊 播放</button>
レモンの匂い <button class="pronunciation-play-btn" data-text="レモンの匂い">🔊 播放</button>
雨が降る <button class="pronunciation-play-btn" data-text="雨が降る">🔊 播放</button>
夜の匂い <button class="pronunciation-play-btn" data-text="夜の匂い">🔊 播放</button>

## 📖 中文翻譯

### 第一段
如果是夢該有多好  
至今仍然夢見你  
就像回去拿忘記的東西一樣  
拂去舊回憶的塵埃

### 副歌
有些幸福是回不來的  
最後是你教會了我  
連說不出口而隱藏的黑暗過去  
也因你而成為光芒

### 第二段
在雨停之前無法回去  
至今你仍是我的光芒  
在黑暗中描繪你的背影  
那輪廓我仍清晰記得

### 橋段
每當遇見無法承受的事物  
就會滿溢不絕的藍色  
那天眺望的街景  
沒有回響腳步聲的世界

### 尾聲
檸檬的香氣  
雨在下  
夜晚的氣息  
檸檬的香氣  
雨在下  
夜晚的氣息

## 🎵 發音練習工具

<div class="pronunciation-widget">
  <div class="text">
    <ruby>夢ならばどれほどよかったでしょう<rt>ゆめならばどれほどよかったでしょう</rt></ruby>
  </div>
  <div class="controls">
    <div class="main-controls">
      <button class="pronunciation-play-btn" data-text="夢ならばどれほどよかったでしょう">🔊 播放歌詞</button>
      <button class="pronunciation-stop-btn">⏹️ 停止</button>
    </div>
    <div class="speed-controls">
      <span class="speed-label">語速：</span>
      <button class="pronunciation-speed-btn" data-speed="0.5">0.5x</button>
      <button class="pronunciation-speed-btn" data-speed="0.8">0.8x</button>
      <button class="pronunciation-speed-btn active" data-speed="1.0">1.0x</button>
      <button class="pronunciation-speed-btn" data-speed="1.2">1.2x</button>
      <button class="pronunciation-speed-btn" data-speed="1.5">1.5x</button>
    </div>
  </div>
</div>

## 🔗 相關內容
- [[posts/japanese-learning/tech-research/pronunciation-demo|發音功能演示]] — 測試發音功能
- [[posts/japanese-learning/lemon-song/index|Lemon 學習專案]] — 完整學習資源
- [[posts/japanese-learning/vocabulary/yume|夢]] — 歌詞中的核心單字
- [[posts/japanese-learning/grammar/naraba-hypothetical-condition|ならば-假設條件]] — 核心文法
- [[posts/japanese-learning/tech-research/pronunciation-integration-guide|發音功能整合指南]] — 了解更多發音功能

